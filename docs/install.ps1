# MediaSim installer - Windows
# Usage:
#   irm https://vegidio.github.io/mediasim/install.ps1 | iex
#   $env:MEDIASIM_INSTALL='cli'; irm https://vegidio.github.io/mediasim/install.ps1 | iex
#   $env:MEDIASIM_VERSION='<tag>'; irm https://vegidio.github.io/mediasim/install.ps1 | iex
#
...
$Mode       = if ($env:MEDIASIM_INSTALL)     { $env:MEDIASIM_INSTALL }     else { 'both' }
$Version    = if ($env:MEDIASIM_VERSION)     { $env:MEDIASIM_VERSION }     else { 'latest' }
$InstallDir = if ($env:MEDIASIM_INSTALL_DIR) { $env:MEDIASIM_INSTALL_DIR } else { Join-Path $env:LOCALAPPDATA 'Programs\mediasim' }
...
if ($Mode -notin @('cli','gui','both')) {
    Write-Fail "invalid MEDIASIM_INSTALL=$Mode (expected: cli, gui, or both)"
}
...
Write-Info "installing mediasim $Tag (windows/$Arch)"

$Tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("mediasim-install-" + [System.Guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($Mode -eq 'cli' -or $Mode -eq 'both') {
        $dir = Get-Asset "mediasim-cli_windows_${Arch}.zip"
        Install-Binary -srcPath (Join-Path $dir 'mediasim.exe') -name 'mediasim.exe' | Out-Null
    }

    if ($Mode -eq 'gui' -or $Mode -eq 'both') {
        $dir = Get-Asset "mediasim-gui_windows_${Arch}.zip"
        $guiPath = Install-Binary -srcPath (Join-Path $dir 'MediaSim.exe') -name 'MediaSim-GUI.exe'

        # Start Menu shortcut for the GUI
        $startMenu = Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs'
        $lnk = Join-Path $startMenu 'MediaSim.lnk'
        try {
