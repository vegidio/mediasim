...
func (m *MediaService) StartComparison(
	ctx context.Context,
	directory string,
	includeImages bool,
	includeVideos bool,
	...
	options := mediasim.DirectoryOptions{
	...
	channel, total := mediasim.LoadMediaFromDirectory(directory, options)

	app := application.Get()
	app.Event.Emit("comparison:progress", map[string]int{"current": 0, "total": total})

	updateCh := mediasim.LoadAndGroupMedia(channel, total, threshold, false)
