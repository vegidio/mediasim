...
type Group struct {
	// Best represents the best media among the similar files. The "best" media is generally the one with higher
	// resolution and the biggest length (if it's a video).
	Best Media `json:"best"`
...
func (g Group) String() string {
	return fmt.Sprintf(`{Best: %s, Media %s}`, g.Best, g.Media)
}
